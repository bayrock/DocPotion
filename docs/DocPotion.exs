    ...
    def buildall(ext \\ ".md") do
        ...
        for file <- layouts do
            ...
            for [block, name] <- includes do
                ...
                document = "#{layout}#{watermark}" |> String.replace(block, partial)
                "#{root}#{file}" |> String.replace(".layout", "") |> File.write!(document)
            end
        end

        if (doc_count = length(layouts)) > 0 do
            {:ok, "Successfully built #{doc_count} document file#{if doc_count > 1 do "s" end}!"}
        else
...
case DocPotion.buildall do
    {:ok, msg} -> msg |> IO.puts
    {:error, msg} -> "Error running DocPotion:\n#{msg}" |> IO.puts
end
